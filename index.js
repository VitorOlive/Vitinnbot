		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '❌ Este comando só pode ser usado pelo Vitinn! ❌',
					ownerB: '❌ Este comando só pode ser usado pelo bot proprietário! ❌',
					admin: '❌ Este comando só pode ser utilizado pelos administradores do grupo! Seu membro comum 😂❌',
       ...
       break
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('Falha, algo deu errado, por favor, tente novamente mais tarde.')
							})
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						if (err) return reply('❌ *Comando desativado* ❌')
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Membro comum @${mentioned[0].split('@')[0]} Se tornou Adm do grupo,Respeitem ele!`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Agora ele @${mentioned[0].split('@')[0]} é um membro comum 😂!`, mentioned, true)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`: @${mentioned[0].split('@')[0]} Foi removido com sucesso!`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Sukses ative o modo simi neste grupo ✔️')
